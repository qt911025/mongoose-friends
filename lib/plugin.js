  ...
  return function friends (schema) {
    ...
    schema.statics.getFriends = function (model) {
      ...
      resolveArgs(arguments, function (conditions, fields, options, cb) {
        ...
        // conditions for embedded friend docs
        conditions[pathName] = conditions[pathName] || {};
        conditions[pathName]['$elemMatch'] = conditions[pathName]['$elemMatch'] || {};
        conditions[pathName]['$elemMatch']['_id'] = model;

        // "accepted" status is mirrored on both sides, but if querying
        // for "pending" or "requested" friends, the query must be reversed
        if (conditions[pathName]['$elemMatch']['status']) {
          switch (conditions[pathName]['$elemMatch']['status']) {
          case Status.Pending:
            conditions[pathName]['$elemMatch']['status'] = Status.Requested;
            break;
          case Status.Requested:
            conditions[pathName]['$elemMatch']['status'] = Status.Pending;
            break;
          }
        }
        ...
        _.extend(conditions[pathName]['$elemMatch'], elemMatch);
    ...
    var getByStatus = function (status) {
      return function (model) {
        resolveArgs(arguments, function (conditions, fields, options, cb) {
          conditions[pathName] = conditions[pathName] || {};
          conditions[pathName]['$elemMatch'] = conditions[pathName]['$elemMatch'] || {};
          conditions[pathName]['$elemMatch']['status'] = status;
    ...
    schema.statics.removeFriend = function (m1, m2, cb) {
      ...
      _.isString(m1) && (m1 = ObjectId(m1));
      _.isString(m2) && (m2 = ObjectId(m2));
     ...
     *
     * @param {Model} model friend doc to remove
     * @param {Function} cb callback to execute on completion or error
     */
